...
Strategy.prototype.authenticate = function(req, options) {
  ...
  Client.bind(username, req.body[self._options.passwordField], function (err) {
    if (err) {
      ...
      if (typeof(self._options.errCallback) === 'function') {
          self._options.errCallback(req, err, username)
      }
      ...
      Client.search(dn, search, function (err, res) {
        ...
        res.on('searchEntry', function(entry) {
          ...
          if (self._options.passReqToCallback) {
            self._verify(req, profile, ver);
          } else {
            self._verify(profile, ver);
          }
        });
